...
class ReconsDataset(torch.utils.data.Dataset):
     ...
     def __getitem__(self, idx):
         ...
         file_name = os.path.join(self.all_data_path, dirs[idx]+'/BW_Nucl.tif')
         I = io.imread(file_name)
         data_gt[:,:,0] = I/255
         file_name = os.path.join(self.all_data_path, dirs[idx]+'/BW_Memb.tif')
         I = io.imread(file_name)
         data_gt[:,:,1] = I/255
...
if __name__ == "__main__":
    ...
    input_size = 1
    output_size = 2
    SRRFDATASET = ReconsDataset(all_data_path="/home/jlh/DBSN/Cropped_images_Nucl_Memb/Training/",
                                transform = ToTensor(),
                                in_size = 256)
    ...
    for epoch in range(5001):
        ...
        torch.save(model.state_dict(),"/home/jlh/DBSN/Models/Nucleus_Membrane/Models_tmp.pkl")
        if epoch in range(0,5001,10):
            torch.save(model.state_dict(),"/home/jlh/DBSN/Models/Nucleus_Membrane/Models_"+str(epoch)+".pkl")
