...
class ReconsDataset(torch.utils.data.Dataset):
     def __init__(self, all_data_path, transform, in_size):
        ...
        self.dirs_data = os.listdir(self.all_data_path+'Tesing_GT_ER/')
     def __len__(self):
         dirs = self.dirs_data
         return len(dirs)          
     ...
     def __getitem__(self, idx):
         ...
         file_name = os.path.join(self.all_data_path+'Tesing_GT_ER/', dirs[idx])
         I_1 = io.imread(file_name)
         file_name = os.path.join(self.all_data_path+'Testing_GT_Adhesion/', dirs[idx])
         ...
         sample = {'image_in': data_in, 'image_ER': I_1, 'image_Adhesion': I_2, 'image_name':dirs[idx][:-4],'max_intensity':max_intensity}
...
if __name__ == "__main__":
    ...
    input_size= 1
    output_size = 1
    SRRFDATASET = ReconsDataset(all_data_path="/home/jlh/DBSN/Cropped_images_ER_Adhesion/",
                                transform = ToTensor(),
                                in_size = 256)   
    ...
    for model_num in range(0,5001,10):
        print(model_num)
        model.cuda(cuda)
        model.load_state_dict(torch.load("/home/jlh/DBSN/Models/ER_Adhesion-DL-intensity/Models_"+str(model_num)+".pkl"))
        model.eval()
        file_path = '/home/jlh/DBSN/Prediction/ER_Adhesion-DL-intensity/Pred_'+str(model_num)+'/'
        ...
        for batch_idx, items in enumerate(test_dataloader):
            ...
            image_1       = items['image_ER']
            image_2       = items['image_Adhesion']
            ...
            image_name = prediction_save_path+'/ER.tif'
            io.imsave(image_name, image_1.detach().cpu().numpy().astype(np.uint16))
            image_name = prediction_save_path+'/Adhesion.tif'
