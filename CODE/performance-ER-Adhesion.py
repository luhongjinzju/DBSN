...
if __name__ == "__main__":
    ...
    gt_path="/media/star/DBSN/DL_Intensity_balance/Data/0-Data_4_training/ER-Adhesion/Cropped_images/Testing_Input/Sample_90.tif"
    dirs = os.listdir(gt_path) 
    p_value_avg = np.zeros((501, 6))
    
    # for pred_num in range(10,5001,10):
        # pred_path = "/media/star/ZJU-DATA/DL_Intensity_balance/Data/0-Data_4_training/ER-Adhesion/Prediction_ER_Adhesion-DL-intensity/Pred_"+ str(pred_num) + "/"
        pred_path = "/media/star/DBSN/DL_Intensity_balance/Data/0-Data_4_training/ER-Adhesion/Prediction_ER_Adhesion-DL-intensity/Suppl_1_Pred_4670/"
        
        data_save = "/media/star/DBSN/DL_Intensity_balance/Data/0-Data_4_training/ER-Adhesion/Performance/Prediction_Intensity/Pred_"+ str(pred_num) + '.xls'
        ...
        for idx in range(len(dirs)):
            ...
            image_name = os.path.join(pred_path, dirs[idx][:-4]+'/Adhesion.tif')
            data_in = Image.open(image_name)
            data_in = np.array(data_in)
            image_name = os.path.join(pred_path, dirs[idx][:-4]+'/ER.tif')
        ...
        for k,p in enumerate(p_value):
            for j,q in enumerate(p):
                    table.write(k,j,q)
        file.save(data_save)
    data_save = "/media/star/DBSN/DL_Intensity_balance/Data/0-Data_4_training/ER-Adhesion/Performance/Prediction_Intensity/avg_all.xls"
    file = Workbook(encoding = 'utf-8')
