...
if __name__ == "__main__":
    ...
    input_size= 1
    # output_size = 1
    SRRFDATASET = ReconsDataset(all_data_path="/home/jlh/20221208/0-SIM-YongXin/ER_Adhesion/",
                                transform = ToTensor())   
    ...
    model_1.cuda(cuda)
    model_1.load_state_dict(torch.load("/home/jlh/20221208/Models/YXSIM_transfer/IB/Models_4280.pkl"))
    model_1.eval()
    
    model_2 = UNet(n_channels=input_size, n_classes=2)
    model_2.cuda(cuda)
    model_2.load_state_dict(torch.load("/home/jlh/20221208/Models/YXSIM_transfer/SC-2/Models_4320.pkl"))
    model_2.eval()
    
    f_p_s = '/home/jlh/20221208/Prediction/YXSIM_transfer/0-Input_ER_Adhesion/'
    file_path = f_p_s + 'ER_Adhesion_IB/'
    folder = os.path.exists(file_path)
    if not folder:
        os.makedirs(file_path)
    file_path_2 = f_p_s + 'ER_Adhesion_IB_SC/'
    ...
    for batch_idx, items in enumerate(test_dataloader):
        ...
        if mv < 0:
            I = I + abs(mv)
        I = I*max_intensity
        prediction_save_path  = file_path_2+image_name[0]+'_ER.tif'
        io.imsave(prediction_save_path , I.detach().cpu().numpy().astype(np.uint16))
        ...
        if mv < 0:
            I = I + abs(mv)
        I = I*max_intensity
        prediction_save_path  = file_path_2+image_name[0]+'_Adhesion.tif'
        io.imsave(prediction_save_path , I.detach().cpu().numpy().astype(np.uint16))
